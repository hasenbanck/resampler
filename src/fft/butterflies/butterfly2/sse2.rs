...
pub(super) unsafe fn butterfly_radix2_stride1_sse2(
    src: &[Complex32],
    dst: &mut [Complex32],
    ...
    unsafe {
        for i in (0..simd_iters).step_by(2) {
            ...
            let b_re = _mm_shuffle_ps(b, b, 0b10_10_00_00); // [b0.re, b0.re, b1.re, b1.re]
            let b_im = _mm_shuffle_ps(b, b, 0b11_11_01_01); // [b0.im, b0.im, b1.im, b1.im]
            ...
            // Mask: select odd lanes (1,3) for add, even lanes (0,2) for sub
...
/// Performs a single radix-2 Stockham butterfly stage for p>1 (out-of-place, SSE2).
///
/// This is the generic version for p>1 cases.
#[target_feature(enable = "sse2")]
pub(super) unsafe fn butterfly_radix2_generic_sse2(
    src: &[Complex32],
    dst: &mut [Complex32],
    stage_twiddles: &[Complex32],
    stride: usize,
    ...
    unsafe {
        for i in (0..simd_iters).step_by(2) {
            ...
            // Store complex numbers using _mm_storel_pd and _mm_storeh_pd.
