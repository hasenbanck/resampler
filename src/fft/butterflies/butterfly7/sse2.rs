...
pub(super) unsafe fn butterfly_radix7_stride1_sse2(
    ...
    unsafe {
        ...
        for i in (0..simd_iters).step_by(2) {
            ...
            let z5_re = _mm_shuffle_ps(z5, z5, 0b10_10_00_00);
            let z5_im = _mm_shuffle_ps(z5, z5, 0b11_11_01_01);
            let w5_swap = _mm_shuffle_ps(w5, w5, 0b10_11_00_01);
            let prod5_re = _mm_mul_ps(w5, z5_re);
            let prod5_im = _mm_mul_ps(w5_swap, z5_im);
            let sub5 = _mm_sub_ps(prod5_re, prod5_im);
            let add5 = _mm_add_ps(prod5_re, prod5_im);
            let t5 = _mm_or_ps(
                _mm_and_ps(select_odd, add5),
                _mm_andnot_ps(select_odd, sub5),
            );

            // Complex multiply: t6 = w6 * z6
            let z6_re = _mm_shuffle_ps(z6, z6, 0b10_10_00_00);
            let z6_im = _mm_shuffle_ps(z6, z6, 0b11_11_01_01);
            let w6_swap = _mm_shuffle_ps(w6, w6, 0b10_11_00_01);
            let prod6_re = _mm_mul_ps(w6, z6_re);
            let prod6_im = _mm_mul_ps(w6_swap, z6_im);
            let sub6 = _mm_sub_ps(prod6_re, prod6_im);
            let add6 = _mm_add_ps(prod6_re, prod6_im);
            let t6 = _mm_or_ps(
                _mm_and_ps(select_odd, add6),
                _mm_andnot_ps(select_odd, sub6),
            );
