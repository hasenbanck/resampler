...
/// AVX implementation of postprocess_fft.
#[cfg(any(
    test,
    not(feature = "no_std"),
...
pub(crate) unsafe fn postprocess_fft_avx(
    output_left_middle: &mut [Complex32],
    output_right_middle: &mut [Complex32],
    twiddles: &[Complex32],
) {
    ...
    unsafe {
        ...
        for chunk in 0..simd_count {
            ...
            let half_sum = _mm256_mul_ps(half, sum);
            let half_diff = _mm256_mul_ps(half, diff);

            let half_sum_plus_real = _mm256_add_ps(half_sum, real);
            let half_diff_plus_imag = _mm256_add_ps(half_diff, imaginary);
            let left_output = _mm256_blend_ps(half_sum_plus_real, half_diff_plus_imag, 0xAA);

            let out_left_ptr = output_left_middle.as_mut_ptr().add(i) as *mut f32;
            _mm256_storeu_ps(out_left_ptr, left_output);

            let half_sum_minus_real = _mm256_sub_ps(half_sum, real);
            let imag_minus_half_diff = _mm256_sub_ps(imaginary, half_diff);
            let right_output = _mm256_blend_ps(half_sum_minus_real, imag_minus_half_diff, 0xAA);
...
/// AVX implementation of preprocess_ifft.
#[cfg(any(
    test,
    not(feature = "no_std"),
    all(feature = "no_std", target_feature = "avx")
...
#[target_feature(enable = "avx")]
pub(crate) unsafe fn preprocess_ifft_avx(
